...
**Data** — `GET /households/{id}/nudges?tone={gentle|direct}` → derived on read (ADR-0007, no storage) from 3 triggers, **in this order**: (1) active recurring `daysUntil(next) <= 5`; (2) non-IOU entry, unsettled, `amount >= 1500 kr` within last 7 days; (3) pairwise net `abs >= 750 kr`. Each nudge: `{ title, body, when, actions[] }` with copy pre-selected by tone. (ADR says balance "crossing 750" as an event vs prototype's simple `>=`; API reconciles — see ambiguities.)
...
| 6 | Balance nudge: ADR "crossing 750 kr" (event) vs prototype simple `>= 750` | **API treats it as `abs(net) >= 750 kr` on read for v1; document "crossing" as future refinement** | Nudges derived on read with no history storage in v1 (ADR-0007); true crossing detection needs prior-state tracking. Flag for API grill. |
...
| 18 | Reminder tone default: JSON prop `direct` vs renderVals fallback `gentle` | **Default `direct`** (matches Settl App/Web harness prop default) | Web harness passes `reminder-tone: "direct"`; the `gentle` fallback only fires when the prop is entirely absent. Now exposed as a per-user setting on `/profil` — persisted on the member (`MeDto.nudgeTone`, PUT `/me`) and passed through as `?tone=`; `direct` remains the default for new members. |
