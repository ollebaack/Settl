...
| 18 | Reminder tone default: JSON prop `direct` vs renderVals fallback `gentle` | **Default `direct`** (matches Settl App/Web harness prop default) | Web harness passes `reminder-tone: "direct"`; the `gentle` fallback only fires when the prop is entirely absent. Expose as a setting later. |
