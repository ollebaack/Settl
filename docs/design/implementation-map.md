...
- **Mobile top header** (sticky, title-integrated): the active household is the **screen title** on the left — square `HouseholdBadge` + name (18px, semibold) + `▾`, tap opens the **Dina hushåll** sheet. The right side is slim: the **bell** (→ Aktivitet, nudge dot when unread) + the acting user's **account avatar** (opens the account menu). The member roster and the balance are **not** repeated here — members live on the household cards / Hushållet header, the balance in the page's net hero. (Superseded the earlier layout, which carried a switcher pill on the left and a *decorative* overlapping member `Avatar` stack on the right; that stack duplicated the acting user, who also appeared as the account avatar, and was a non-tappable fake target.)
