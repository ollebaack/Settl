...
`Lån` (`iou`) path turned out to be redundant: the `Allt` split preset added in
...
- **We will remove `EntryType.Iou` and the `FromMemberId`/`ToMemberId` columns.** Every
  entry becomes an `expense`; "one owes all" is the `Allt` amount-split preset
...
- **Seed-only data rewrite:** the two `DbInitializer` `Iou(...)` calls become `Expense` +
  `Allt`. The column-dropping EF migration carries a **defensive backfill**
...
settlements are already first-class events (ADR-0007) and `Allt` covers one-owes-all.
