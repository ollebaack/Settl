...
/// <summary>
/// Integration tests for hard-deleting empty households
/// (docs/specs/household-ownership.md) — a carve-out to the household-ownership spec's soft-only rule. Owner-only;
...
public class HouseholdDeleteTests
{
    ...
    public async Task Delete_household_with_only_a_recurring_template_is_409()
    {
        ...
        // A template but no posted entries still counts as activity (household-ownership spec empty def).
    ...
    public async Task Delete_empty_household_removes_other_members_and_pending_invites()
    {
        ...
        // Owner overrides extra members (household-ownership spec): Sam has joined, and there's a pending invite.
