...
// Sam joins via a real invite (ADR-0005 — households are invite-only), so the
...
test('settle up with a person closes the pair', async ({ page }) => {
  ...
  // focused book dashboard (contacts-phone-sms spec).
...
// ADD-YOUR-NUMBER NUDGE (contacts-phone-sms spec). The flip side of Swish pay: when you are OWED by someone and
...
// HOME "ADD YOUR NUMBER" BANNER (contacts-phone-sms spec). On the overview, when you're owed money in a book and
...
// account whose number is set via the API (contacts-phone-sms spec — the single number doubles as the Swish
