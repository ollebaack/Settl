/**
 * A single household's dashboard (implementation-map §2.1): net balance hero,
 * per-person balance rows (→ Settle-up sheet), a mobile-only "På gång" upcoming
 * rail of ghost cards (→ Recurring detail), and a "Senaste" recent list
 * (→ /ledger). All server state comes from the shared TanStack Query hooks; the
 * screen renders and calls, never computes (ADR-0006).
 *
 * This is the "in a book" view. It is rendered directly at `/` for a
 * single-household user (ADR-0019: the overview never appears for them) and at
 * `/hushall/$id` for a multi-household user who drilled into one book from the
 * overview.
 */
